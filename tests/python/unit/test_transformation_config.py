...
class TestTransformationConfig:
    def test_load_input_parameters(self):
        ...
        dku_config = TransformationConfig()
        assert dku_config.load_input_parameters(valid_config, input_dataset_columns) is None
        ...
        with pytest.raises(DSSParameterError):
            _ = dku_config.load_input_parameters(invalid_target_config, input_dataset_columns)
        invalid_time_config = {"time_column": "date_different", "target_columns": ["value1", "value2"],
                               "frequency_unit": "D"}
        with pytest.raises(DSSParameterError):
            _ = dku_config.load_input_parameters(invalid_time_config, input_dataset_columns)
    ...
    def test_long_format(self):
        ...
        dku_config = TransformationConfig()
        assert dku_config.load_input_parameters(config, input_dataset_columns) is None

        config["timeseries_identifiers"] = []
        with pytest.raises(DSSParameterError):
            _ = dku_config.load_input_parameters(config, input_dataset_columns)
